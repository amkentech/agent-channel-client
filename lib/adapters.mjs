...
/** Merge our hook entries into an existing hooks JSON without clobbering unrelated hooks (dedup by recognizing our own hook scripts). */
export function mergeHooks(existing, ours) {
  ...
  for (const [ev, entries] of Object.entries(ours.hooks || {})) {
    ...
    // "Ours" is any command running one of this package's hook scripts, wherever it was installed FROM. The
    // path-prefix alternation alone missed our own hooks wired from an unpacked tarball (…\pack-test\package\
    // hooks\inbox.mjs): same hook, different install path, so a re-wire ADDED instead of replaced and every
    // event fired twice. The filename list is closed on purpose — a third party's hooks/lint.mjs stays theirs.
    const isOurs = (h) => /(agent-channel|\.agentchan[\\/]client)[\\/]hooks[\\/]|[\\/]hooks[\\/](inbox|btw|notify|secret-guard|claude-status|statusline)\.mjs/i.test(h.command || "");
