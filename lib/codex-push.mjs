...
/** Codex app-server versions have returned thread/loaded/list data in two shapes:
 *  an array of thread objects, and (0.150+) an array of thread-id strings. */
export function selectLoadedThread(loaded) {
  const threads = Array.isArray(loaded?.data) ? [...loaded.data] : [];
  if (!threads.length) return { threadId: null, reason: "no loaded thread" };
  const objects = threads.filter((thread) => thread && typeof thread === "object" && thread.id);
  const strings = threads.filter((thread) => typeof thread === "string" && thread);
  if (objects.length === threads.length) {
    objects.sort((a, b) => (b.recencyAt || b.updatedAt || 0) - (a.recencyAt || a.updatedAt || 0));
    return { threadId: objects[0].id, reason: null };
  }
  if (strings.length === 1 && strings.length === threads.length) return { threadId: strings[0], reason: null };
  if (strings.length > 1 && strings.length === threads.length) {
    return { threadId: null, reason: "multiple loaded Codex threads; refusing to choose an unverified target" };
  }
  return { threadId: null, reason: "unrecognized loaded-thread response" };
}

export function selectLoadedThreadId(loaded) {
  return selectLoadedThread(loaded).threadId;
}
  ...
  return new Promise((resolve) => {
    ...
    ws.on("open", async () => {
      try {
        ...
        const tid = selection.threadId;
        if (!tid) {
          clearTimeout(t);
          const hint = selection.reason === "no loaded thread" ? " (start Codex with --remote " + url + ")" : "";
          return finish({ ok: false, reason: selection.reason + hint });
        }
        if (mode === "inject") {
